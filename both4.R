...
#good<-read.csv("/Users/samsonniemotka/Desktop/SLU/Independent Studies/Dr. W/COVID/template.csv")
#poo<-read.csv("/Users/samsonniemotka/Desktop/SLU/Independent Studies/Dr. W/COVID/templatebad.csv")
...
compat<-function(data){
   ...
   if(nrow(data)<15){
      return("Error: Data needs at least 15 rows for analysis.")
   }
...
###########################################################################################
anombreak.covid.mort_upload<-function(data, alpha=0.05, max_anoms=0.2, n.break=7, yaxis="Y axis label", xaxis = "Date", breaks = "1 year"){
   ...
   #### End date fix ####  
      ...
      plotz<-ggplot() +
            ...
            scale_x_date(expand = c(0.05, 0.05), date_labels = "%b%d", date_breaks = breaks) +
